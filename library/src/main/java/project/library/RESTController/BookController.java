...
public class BookController {
	...
	public ResponseEntity<?> addBooks(@RequestBody List<Book> books) {
		...
		return addedBooks != null ? ResponseEntity.status(HttpStatus.CREATED).body(addedBooks)
				: ResponseEntity.status(HttpStatus.BAD_REQUEST).body("Failed to add books");
	}
	...
	public ResponseEntity<?> renewBook(@RequestParam int borrowedBookId, @RequestParam int userId) {
		logger.info("User {} renewing book {}", userId, borrowedBookId);
